...
Процедура ПроверкаАвторизацииПользователя1С()
	...
	Если ЗначениеЗаполнено(Пользователь1С) И ЗначениеЗаполнено(Пароль1С) Тогда
		ПользовательИБ = Пользователи.ПрочитатьПользователяИБ(Пользователь1С.ИдентификаторПользователяИБ);
		СлужебныйАвторизация1СКорректна = Пользователи.СохраняемоеЗначениеСтрокиПароля(Пароль1С) = ПользовательИБ.СохраняемоеЗначениеСтрокиПароля;
	Иначе
		Запрос = Новый Запрос;
		Запрос.Текст = 
			"ВЫБРАТЬ
...
Процедура АвторизацияGitПроверкаНаСервере() 
	...
	Если ЗначениеЗаполнено(ТипАвторизацииGit) И ЗначениеЗаполнено(EmailGit) И ЗначениеЗаполнено(ЛогинGit) Тогда
		...
		Если ДанныеЗаполнены Тогда 
			//СлужебныйДанныеХранилища = ВерсВО_ОбщегоНазначения.ПолучитьСведенияОРепозитории(АдресРепозиторияGit, ТипАвторизацииGit, СтруктураАвторизации, Ложь);
			СлужебныйАвторизацияГитКорректна = Истина;
			//СлужебныйДанныеХранилища.АвторизацияУспешна;
		КонецЕсли;
	КонецЕсли;
	...
	Иначе 
		ФайловаяСистема.УдалитьВременныйКаталог(СлужебныйДанныеХранилища.ПутьКВременномуКаталогу);
